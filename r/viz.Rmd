...
plot_df %>% 
    filter(mode == 'random', c == 4) %>% 
    mutate(approx = PA(lambda, alpha, c),
           log_lambda = round(log2(lambda)),
           log_lambda = factor(log_lambda, levels = -(1:5)*2),
           x = ifelse(round(alpha*100) %% 5 == 0, x, NA)) %>% 
    ggplot() + 
    aes(x = alpha) + 
    geom_line(aes(y = approx, group = log_lambda, color = log_lambda), size = 1) + 
    geom_point(aes(y = x, fill = log_lambda), pch = 21) + 
    theme_bw() + 
    scale_color_brewer(palette = 'Dark2') + 
    scale_fill_brewer(palette = 'Dark2') + 
    xlab(expression(alpha)) + 
    ylab(expression(x)) + 
    guides(fill = guide_legend(title = expression(log~lambda)),
           color = FALSE)

ggsave('fig/PA.pdf', width = 6, height = 4)
