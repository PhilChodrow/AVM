...
\section{Analysis}
	...
	\subsection{Approximating the Voter Term}
		...
		We fix a node $v$, and suppose that, at time $t$, $v$ changed its opinion from $0$ to $1$ due to either a mutation or a voting step. \todo{have already used $v$ for the voting term}
		...
		\begin{enumerate}
			...
			\item \textbf{Mean Field:} We assume that nodes other than $v$ have degree distributions governed by the global network average $\mathbf{x}$. \todo{should say something to justify why we aren't considering reactivated edges when $v$ votes.}
		\end{enumerate}
		...
		\begin{figure}
			\centering     
				...
				\end{tikzpicture}
			\caption{Initial state in our approximation scheme, with $J_0 = 1$ and $K_0 = 3$.}\label{fig:init_diagram}
		\end{figure}
